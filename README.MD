...
m3u8Download('test/t.m3u8', {
  debug: true,
  filenmae: '测试视频',
  ffmpegPath: ffmpegStatic, // 使用 ffmpeg-static 包提供的路径
});

// 方式二：指定系统安装的 ffmpeg 完整路径
m3u8Download('test/t.m3u8', {
  debug: true,
  filenmae: '测试视频',
  ffmpegPath: '/usr/local/bin/ffmpeg', // Linux/Mac 系统路径
  // ffmpegPath: 'C:\\ffmpeg\\bin\\ffmpeg.exe', // Windows 系统路径
});
```
