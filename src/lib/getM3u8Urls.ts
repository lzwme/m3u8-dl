...
export async function getM3u8Urls(opts: GetM3u8UrlsOption) {
  ...
  if (options.maxHtmlSize && html.length > options.maxHtmlSize) {
    logger.warn(
      'HTML 内容过大，跳过处理:',
      color.yellow(options.url),
      `${(html.length / 1024 / 1024).toFixed(2)}MB`,
      `(限制: ${(options.maxHtmlSize / 1024 / 1024).toFixed(2)}MB)`
    );
    return m3u8Urls;
  }
  ...
  if (m3u8Urls.size === 0 && options.deep > 0) {
    ...
    const taskList = Array.from(subPageUrls.entries()).map(([href, text]) => {
      return async () => {
        // 达到最大 URL 数量限制，不再继续
        if (options.maxUrls && m3u8Urls.size >= options.maxUrls) return;
    ...
    await concurrency(taskList, Math.max(1, +options.concurrency || 3));
  }
