...
export class PipixiaParser extends BaseParser {
  public static override async parse(url: string): Promise<ApiResponse<VideoInfo>> {
    try {
      ...
      if (!loc.includes('/item/')) return PipixiaParser.error(400, '无效的URL格式');

      const idMatch = loc.match(/item\/([^?]*)/);
      if (!idMatch?.[1]) return PipixiaParser.error(400, '无法解析视频 ID');
      ...
      if (!data) return PipixiaParser.error(400, '未找到视频数据');

      const item = data.data.cell_comments[0].comment_info.item;
      const videoUrl = item.video.video_high.url_list[0].url;

      if (!videoUrl) return PipixiaParser.error(201, '未找到视频URL');

      return PipixiaParser.success<VideoInfo>({
      ...
      return PipixiaParser.error(500, `解析失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
}
